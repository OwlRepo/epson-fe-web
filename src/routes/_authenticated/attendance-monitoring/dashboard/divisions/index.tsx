    ...
    return (
        <CardSection headerLeft={<CardHeaderLeft />} headerRight={<CardHeaderRight />}>
            ...
            <div className='grid grid-cols-1 md:grid-cols-4 gap-8'>
                {
                    isConnected && !isLoading ? data.filter(d => d.name).map((division) => (
                        <Link to={`/attendance-monitoring/dashboard/divisions/$divisionId`} params={{ divisionId: division.name }} key={division.name}>
                            <DepartmentCard
                                title={division.name}
