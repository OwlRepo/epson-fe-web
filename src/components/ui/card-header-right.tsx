...
export default function CardHeaderRight({ title, subtitle, clockedIn, clockedOut }: CardHeaderRightProps) {
    ...
    return (
        <div className="flex flex-col items-end gap-1">
            ...
            {typeof subtitle === 'string' || subtitle ? <p className='text-muted-foreground text-sm'>{subtitle}</p> : <div className='flex flex-row space-x-10'>
                {clockedIn && <div className='flex flex-row space-x-2'>
                    <p>Clocked In:</p>
                    <b className="text-positive">{countShortener(clockedIn ?? 0)}</b>
                </div>}
                {clockedOut && <div className='flex flex-row space-x-2'>
                    <p>Clocked Out:</p>
